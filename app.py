...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'test_app_secret_2024_xyz_v4')
...
DEFAULT_CONFIG = {"admin_password": "admin123"}
...
def teacher_register():
    if request.method == 'POST':
        ...
        tid = str(uuid.uuid4())
        con.execute('INSERT INTO teachers (id,name,username,password_hash,email,created_at,time_limit,question_count) VALUES (?,?,?,?,?,?,?,?)',
...
def teacher_export_results():
    ...
    return _build_excel(results, cat)
...
def admin():
    ...
    con.close()
    return render_template('admin.html', config=config, teachers=teachers)
...
def toggle_teacher():
    data = request.json
    if data.get('password') != load_config().get('admin_password','admin123'):
        return jsonify({'error':"Noto'g'ri parol!"}),403
    con = get_db()
